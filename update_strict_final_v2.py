...
# Same new function content as before (truncated here for brevity in logic description, but full string is passed)
new_func = r'''def generate_card_zip(char, manager):
    ...
    def create_card_3():
        ...
        # 3 (Left)
        p = get_img(2)
        if p: bg.paste(load_image_cover(p, (300, 600)), (40, 100))
        # 4 (Center Top)
        p = get_img(3)
        if p: bg.paste(load_image_cover(p, (300, 290)), (360, 100))
        # 5 (Right Top)
        p = get_img(4)
        if p: bg.paste(load_image_cover(p, (300, 290)), (680, 100))
        # 6 (Center Bot)
        p = get_img(5)
        if p: bg.paste(load_image_cover(p, (300, 290)), (360, 410))
...
try:
    ...
    with open(target_file, 'r', encoding='mbcs') as f: # Windows fallback
        content = f.read()

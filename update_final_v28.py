...
# THE V28 CODE
v28_code = r'''def generate_card_zip(char, manager):
    import io
    import zipfile
    import os
    ...
    # --- V28 POLISH (Shadows, Top Align Memo, Card 3 Title) ---
    print(f"DEBUG: V28 - {char.get('name')}")
    ...
    def inputs_paste_shadowed(base_img, fg_img, xy, blur=15, offset=(5,5)):
        # Helper to paste an already masked object with a shadow
        # Create shadow from alpha channel of fg_img
        if not fg_img: return
        
        alpha = fg_img.split()[-1]
        # Create shadow image
        shadow = Image.new("RGBA", fg_img.size, (0,0,0,0))
        # Fill with black based on alpha
        ...
        shadow_layer.paste(shadow, (margin, margin), mask=alpha)
        
        # Blur
        shadow_layer = shadow_layer.filter(ImageFilter.GaussianBlur(blur))
        
        # Reduce opacity? "Semiparent black shadow" -> Blur usually handles it but let's lower alpha
        # shadow_layer.putalpha(150)? It's already alpha blended.
        
        # Paste shadow
        sx = xy[0] - margin + offset[0]
        sy = xy[1] - margin + offset[1]
        
        base_img.alpha_composite(shadow_layer, (sx, sy))
    ...
    def create_card_1():
        ...
        # V28: Image 2 Right Align to Text Area Right Edge.
        # Text Area Margins: 40px left, 40px right (from the frame edge? or page edge?)
        # Let's assume Text Block is X=50 to X=974? (Margin 50)
        # Or if "Text Area" means the beige box in the template...
        # Let's target X=80 as before, but with padding inside.
        
        # Let's set Image 2 Right Edge to 964 (same as V25 which user liked somewhat, but asked for "Exact").
        im2_right = 964
        im2_w = 320
        im2_left = im2_right - im2_w # 644
        
        img1_box = (125, 107, 360, 360)
        img2_box = (im2_left, 107, im2_w, 520)
        ...
        # V28: Memo Block
        # Area: X=80 to X=964 (to match image right 964). Total Width = 884.
        # User wants "Equal Margins (40px) from text area ends".
        # So text starts at 80+40 = 120. Ends at 964-40 = 924.
        # Text Width = 804.
        
        memo_area_start_x = 80
        memo_padding = 40
        text_x = memo_area_start_x + memo_padding
        text_w = 884 - (memo_padding*2) # 804
        
        memo_y = 750 # Base Y for area top
        ...
        # V28: Top Align for body (resume "normal" wrapped text)
        # Gap 1 line ~ 30px
    ...
    # --- 2. STATS CARD (V28) ---
    def create_card_2():
        print("DEBUG: Creating Card 2 (V28)")
        ...
        # V28: Transp 100/255
        draw_radar(bg, (256, chart_y), chart_r, char.get('stats', {}), lbl_b, (30, 136, 229, 100), (30, 136, 229, 255))
        
        lbl_p = ["積極性", "協調性", "慎重さ", "適応力", "精神力", "寛容さ", "道徳・倫理観", "社交性"]
        draw.text((768, 120), "性格傾向", font=f_med, fill="#3e2723", anchor="mm")
        draw_radar(bg, (768, chart_y), chart_r, char.get('personality_stats', {}), lbl_p, (229, 57, 53, 100), (229, 57, 53, 255))
    ...
    # --- 3. GALLERY CARD (V28) ---
    def create_card_3():
        print("DEBUG: Creating Card 3 (V28)")
        if os.path.exists(TEXTURE_PATH):
            bg = Image.open(TEXTURE_PATH).convert("RGBA").resize(CANVAS_LANDSCAPE)
        else:
            bg = Image.new("RGBA", CANVAS_LANDSCAPE, (250, 245, 230, 255))
        ...
        # V28: Shadows for all images using inputs_paste_shadowed
        
        if p2:
             i = load_image_masked(p2, (w, h_tall), radius=R, centering=(0.5, 0.5))
             inputs_paste_shadowed(bg, i, (60, y_start))
             
        if p3:
             i = load_image_masked(p3, (w, h_sq), radius=R)
             inputs_paste_shadowed(bg, i, (368, y_start))
             
        if p4:
             i = load_image_masked(p4, (w, h_sq), radius=R)
             inputs_paste_shadowed(bg, i, (676, y_start))
             
        y_bot = y_start + h_sq + gap 
        if p5:
             i = load_image_masked(p5, (w, h_sq), radius=R)
             inputs_paste_shadowed(bg, i, (368, y_bot))
        ...
        # V28: Header Move +20px (60 -> 80)
        fname = char.get('first_name', '')
        lname = char.get('last_name', '')
        full = f"{fname}・{lname}" if fname and lname else char.get('name', '')
        draw.text((512, 80), full, font=f_med, fill="#3e2723", anchor="mm")

        en = char.get('name_en', '')
        if en: draw.text((512, 50), en, font=f_med, fill="#3e2723", anchor="mm")
        
        # V28: Huge Title "【人間関係】" at top of block
        # Block starts y=408.
        draw.text((rel_x + w//2, rel_y - 20), "【人間関係】", font=f_med, fill="#3e2723", anchor="mb") 
...
if match:
    new_content = content[:match.start()] + v28_code + "\n\n" + content[match.end():]

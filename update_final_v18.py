...
# THE V18 FUNCTION
v18_code = r'''def generate_card_zip(char, manager):
    import io
    import zipfile
    import os
    ...
    print(f"DEBUG: V18 Restoration Logic")
    ...
    def draw_text_wrapped(draw, text, x, y, max_h, max_w, font, fill, align="left"):
        ...
        # Basic Top-Down flow for Bio usually, but let's default to standard flow unless centered requested
        # Reviewing V15: It had vertical centering logic `start_y = y + (max_h - total_h) // 2`
        # Reviewing Image: Text seems top-aligned relative to the "Detail" header?
        # The user said "text layout is NOT broken" in the reference image (which is seemingly V13/V14ish?).
        # In V14/V15:
        #   start_y = y + (max_h - total_h) // 2 
        #   This CENTERS vertically within the box.
        #   If user complains about layout BROKEN, maybe they WANT it centered (V15 logic) or NOT centered (standard)?
        # The provided image shows nice wrapping.
        # I will stick to Vertical Centering as it seems to be the "good" state passed down.
    ...
    # --- 1. PROFILE CARD (V16/V17 Logic Kept) ---
    def create_card_1():
        print(f"DEBUG: Creating Card 1 for {char.get('name')}")
        details = char.get('details', {})
        race = details.get('race', '人間')
        
        # Logic: Demon -> White top, Others -> Dark top.
        if race == "魔族":
            txt_col = "#FFFFFF"
        else:
            txt_col = "#333333"
        memo_col = "#333333" # Always dark
    ...
    # --- 2. STATS CARD (RESTORED TO REFERENCE IMAGE STYLE) ---
    def create_card_2():
        print("DEBUG: Creating Card 2 (Restored)")
        ...
        # RESTORED CORNER DECORATIONS (V15 logic)
        ...
        # Text wrapping with vertical centering
        draw_text_wrapped(draw, bio, dx, dy+60, 200, 900, get_font(16), "#212121")
        
        # FOOTER: Reference image shows NO FOOTER. Removed for exact match.
        return bg
...
if start_pos != -1 and end_pos != -1:
    ...
    new_content = pre + v18_code + "\n\n" + post
    with open(target_file, 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("V18 Card 2 Restoration Applied.")
else:

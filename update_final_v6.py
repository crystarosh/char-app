...
# --- NEW FUNCTION CODE V6 ---
# 1. Added Debug Print in Card 3
# 2. Card 1: Memo font color -> Force Dark
# 3. Card 1: Memo Vertical Center
# 4. Card 1: ImageOps.fit with centering=(0.5, 0.5)
...
new_func_code = r'''def generate_card_zip(char, manager):
    ...
    # Revised Wrap with Vertical Center
    def draw_text_wrapped(draw, text, x, y, max_w, font, fill, align="left", max_h=None):
        ...
        # Logic: If max_h is provided, CENTER the block in that height
        if max_h:
            offset = (max_h - total_h) // 2
            start_y += offset # Move start down
    ...
    def create_card_1():
        ...
        # Images: Revised with Center Crop
        if len(char['images']) > 0:
            i1 = load_image_cover(char['images'][0], (450, 450)) # Implies centered fit
            img.paste(i1, (50, 100))
            
        if len(char['images']) > 1:
            i2 = load_image_cover(char['images'][1], (400, 650)) # Implies centered fit
        ...
        memo_col = "#000000" # Explicit overwrite for visibility
        memo = details.get('profile_memo', details.get('memo', ''))
        # Memo Area: Height ~ 140px. Y start ~ 860.
        draw_text_wrapped(draw, memo, 80, 860, 860, get_font(22), memo_col, max_h=140)
        return img
    ...
    def create_card_3():
        ...
        # --- DEBUG START ---
        print(f"DEBUG: Character Name: {char.get('name')}")
        print(f"DEBUG: Images List Content: {imgs}")
        print(f"DEBUG: Images List Length: {len(imgs)}")
        # --- DEBUG END ---
...
try:
    ...
    if pattern.search(content):
        ...
        with open(target_file, 'w', encoding='utf-8') as f:
            f.write(new_content)
        print("Function generate_card_zip updated successfully V6 with Debugs and Fixes.")
    else:
        print("Regex match failed. Falling back.")
        # Fallback Logic can be added here if needed but regex should work on normalized file.
        # Let's just rely on regex for V6 since we normalized in V5/V4 steps.

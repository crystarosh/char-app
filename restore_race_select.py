...
# Goal: Restore Race Selectbox logic.
# Previously it might have been split 'race' and 'role_text' or combined.
# The template selection logic uses `details.get('race')`.
# The current form has `role = st.text_input("種族 / 職業")`.
# We need to explicitly ask for "Race" for template matching.
...
update_code = r'''def render_register_page(manager, edit_char_id=None):
    ...
    with st.container():
        ...
        with col1:
            # RESTORED: Race Selection + Role Input
            # Template map keys: "人間", "魔族", "聖族", "その他"
            # It's better to store "race" for template separately from displayed "role".
            ...
            race_in = st.selectbox("種族 (テンプレート選択用)", race_opts, index=race_opts.index(curr_race))
            
            # Role (Display Text) - Pre-fill with existing role or Race if empty
            curr_role = details.get('role', '')
            role_in = st.text_input("種族・職業表示 (カード記載用)", value=curr_role, placeholder="例: 人間 / 騎士")
...
# The rest of the function needs to be included to close it properly, OR we insert/replace carefully.
# The user's request is small ("Race List Missing").
# I will replace the START of the function up to the color picker part where I stopped.
# But wait, I need to ensure the `submit` logic also SAVES the `race_in`.
# The current code saves `role` but maybe not `race` explicitly if I changed logic.
# Ah, in V1 (previous code), `role` was just a text input.
# I need to verify how `race` is saved in the submit block.
# In the previous `view_file`, line 373: `'role': role,`. `race` is NOT in the submit block?
# Wait, `create_card_1` uses `details.get('race', '人間')`.
# If `race` is not saved in `details`, it defaults to '人間'.
# So I MUST update the SUBMIT logic too.

# I will write the FULL function to be safe.

full_function_code = r'''def render_register_page(manager, edit_char_id=None):
    title = "📝 新規キャラクター登録"
    existing_data = {}
    ...
    if submitted:
        ...
        new_char = {
            ...
            "details": {
                "race": race_in, # SAVING RACE
                "age": age,
                "role": role_in, # SAVING ROLE
                "origin": origin,
...
if match:
    ...
    new_content = pre + full_function_code + "\n\n" + post
    with open(target_file, 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("RESTORED: Race Dropdown & Role Input in Register Page.")
else:

...
# THE V30 CODE
v30_code = r'''def generate_card_zip(char, manager):
    import io
    import zipfile
    import os
    ...
    # --- V30 FINAL TWEAKS (Shadow 120, Radar Alpha, Prof Center) ---
    print(f"DEBUG: V30 - {char.get('name')}")
    ...
    def inputs_paste_shadowed_strict(base_img, fg_img, xy, blur=7, offset=(0,0), alpha_val=130):
        # V30: Parametrized Shadow
        if not fg_img: return
        
        alpha = fg_img.split()[-1]
        w, h = fg_img.size
        
        margin = blur * 3 # ample margin
        ...
        # Paste silhouette centered in temp layer
        ...
        # Calculate paste pos
        # xy is target top-left of image
        # Shadow starts at xy - margin + offset
    ...
    def create_card_1():
        ...
        # V30: Symmetric Margins.
        # User said "Lower beige area ... equal blank space from left and right of the whole image".
        # This implies Centered Layout relative to the Card (1024 width).
        # Standard Margin 60 seems good? Or 80?
        # Let's keep 60 (Width 904).
        
        margin_x = 60
        area_w = 1024 - (margin_x*2) # 904
        area_right = 1024 - margin_x # 964
        
        img1_box = (125, 107, 360, 360)
        
        # Image 2 Right Align to area_right (964)
        im2_w = 320
        ...
        # V30: Memo Area
        # Symmetric left/right margins from PAGE edge (60 on Page => 60L 60R).
        # Image 2 aligned to 964 (Right of area).
        
        # Wait, if V29 was 100 on inside? "equal margins".
        # Let's assume standard symmetric page.
        ...
        title_text = "【簡易設定】"
        draw.text((memo_x + 50, memo_y), title_text, font=title_font, fill=memo_col, anchor="la")
        
        # Indent body
        draw_text_wrapped(draw, details.get('memo', ''), memo_x + 50, memo_y+35, 9999, area_w - 100, body_font, memo_col, valign="top")
        
        return img

    # --- 2. STATS CARD (V30) ---
    def create_card_2():
        print("DEBUG: Creating Card 2 (V30)")
        ...
        # V30: Lighter Alpha (~70?)
        draw_radar(bg, (256, chart_y), chart_r, char.get('stats', {}), lbl_b, (30, 136, 229, 70), (30, 136, 229, 255))
        
        lbl_p = ["積極性", "協調性", "慎重さ", "適応力", "精神力", "寛容さ", "道徳・倫理観", "社交性"]
        draw.text((768, 120), "性格傾向", font=f_med, fill="#3e2723", anchor="mm")
        draw_radar(bg, (768, chart_y), chart_r, char.get('personality_stats', {}), lbl_p, (229, 57, 53, 70), (229, 57, 53, 255))
    ...
    # --- 3. GALLERY CARD (V30) ---
    def create_card_3():
        print("DEBUG: Creating Card 3 (V30)")
        if os.path.exists(TEXTURE_PATH):
            bg = Image.open(TEXTURE_PATH).convert("RGBA").resize(CANVAS_LANDSCAPE)
        else:
            bg = Image.new("RGBA", CANVAS_LANDSCAPE, (250, 245, 230, 255))
        ...
        # V30: Shadow 120deg (Bottom Right). Offset (3.5, 6)?
        # 120deg: x=dist*cos(120)=-3, y=dist*sin(120)=5.1 
        # Wait, if angle is "Light Source", shadow is opposite (-60).
        # If angle is "Drop Shadow Angle", usually standard is 120 (Top Left Light).
        # Standard Drop Shadow: +x, +y.
        # Let's use (4, 4) approx.
        # Opacity 51% (130). Blur 7.
        ...
        # V30: Title "【人間関係】" Above Icons.
        # Icons start at rel_y + 40 ? No.
        # Icons are drawn in loop.
        # Let's draw title at rel_y.
        draw.text((rel_x + w//2, rel_y), "【人間関係】", font=f_med, fill="#3e2723", anchor="mb") 
        ...
        for i, r in enumerate(rels):
             ...
             row = i // 2
             bx = rel_x + col * 144
             by = rel_y + row * 144 + 40 # Shift down to make room for Title?
             
             # Actually spacing is tight. Block Height = 288.
             # Title at 0.
             # Icons need to fit below.
             # Let's shift icons down by 30px?
             
             by_adj = by
             
             rtype = r.get('type', '').split('/')[0]
             draw.text((bx+72, by_adj+20), rtype, font=get_font(14), fill="#5d4037", anchor="mm")
             ...
             bg.paste(ic, (bx+42, by_adj+40), ic)
             ...
             draw.text((bx+72, by_adj+115), tname, font=get_font(16), fill="#3e2723", anchor="mm")
...
if match:
    new_content = content[:match.start()] + v30_code + "\n\n" + content[match.end():]

...
# THE V14 FUNCTION
v14_code = r'''def generate_card_zip(char, manager):
    import io
    import zipfile
    import os
    ...
    def load_image_masked(path, size, radius=0, centering=(0.5, 0.5)):
        try:
            ...
            if radius > 0:
                ...
                # Apply mask implies creating a new transparent image and pasting with mask
    ...
    def create_card_1():
        ...
        # 1. BASE: Use Template as Background (No Sandwich)
        template_map = {
        ...
        # 2. IMAGES ON TOP
        # Strict Coords & Size
        # Image 1: (110, 110), 400x400
        if len(char['images']) > 0:
            if char['images'][0] and os.path.exists(char['images'][0]):
                # Profile usually square, no radius requested here explicitly but "Corner round mask also used" in prompt snippet
                # However, Profile frames are usually square. I will use radius=0 to ensure full fill.
                # Use centering (0.5, 0.5)
                i1 = load_image_masked(char['images'][0], (400, 400), radius=0, centering=(0.5, 0.5))
                img.paste(i1, (110, 110), i1)
                
        # Image 2: (720, 110), 240x600 (User said 720, 240x600)
        # Use centering (0.5, 0.2)
    ...
    def create_card_2():
        ...
        # draw_decorations(draw, 1024, 768) # Assuming this isn't strictly needed or handled by base? 
        # User didn't specify decorations removal, but standard texture usage.
        # Adding simple border if needed.
        draw.rectangle((40, 40, 1024-40, 768-40), outline="#5d4037", width=2)
        ...
        # Reuse local radar? (Assuming context has it or I need to redefine/import)
        # Since I'm replacing the whole function block, I need draw_radar defined or accessible.
        # It was defined inside generate_card_zip in previous versions.
        # I must include it here.
        # (Included below)
        
        # Placeholder for drawing radar...
        # Wait, I need to include draw_radar in this V14 code block!
        pass 
        return bg
    ...
    def create_card_3():
        ...
        # 1. BASE: Template (SNS-3.png) as Background
        gp = os.path.join("templates", "SNS-3.png")
        if os.path.exists(gp):
            bg = Image.open(gp).convert("RGBA").resize(CANVAS_LANDSCAPE)
        else:
            bg = Image.new("RGBA", CANVAS_LANDSCAPE, (250, 245, 230, 255))
        ...
        # 2. IMAGES ON TOP
        # Coords (Original logic):
        # p2 (Tall): (40, 100) 300x600. Centering (0.7, 0.5). Radius 30.
        # p3 (MidTop): (360, 100) 300x290. Centering (0.5, 0.5). Radius 30.
        # p4 (RightTop): (680, 100) 300x290. Radius 30.
        # p5 (MidBot): (360, 410) 300x290. Radius 30.
        ...
        # 3. TEXT ADJUSTMENTS
        # Name OUTSIDE frame (Top), smaller font
        en = char.get('name_en', '')
        if en: 
             # Assuming frame is top? Or just name at top.
             # Position: (512, 40) is top center.
             # Font size: use f_med (30) instead of f_romaji (40)?
             draw.text((512, 30), en, font=f_med, fill="#3e2723", anchor="mm")
        ...
        # Footer "Legend of Crystarosh"
        draw.text((980, 750), "Legend of Crystarosh", font=get_font(12), fill="#5d4037", anchor="rm")
            
        return bg

    # Redefine create_card_2 fully with the draw_radar calls included
    def create_card_2_full():
        print("DEBUG: Creating Card 2")
        TEXTURE_PATH = "C:/Users/sweet/.gemini/antigravity/brain/f632eb69-1385-4e0a-bf7b-2cc9ec5d7899/old_paper_texture_1767013103684.png"
        if os.path.exists(TEXTURE_PATH):
            bg = Image.open(TEXTURE_PATH).convert("RGBA").resize(CANVAS_LANDSCAPE)
...
if start_pos != -1 and end_pos != -1:
    ...
    new_content = pre + v14_code + "\n\n" + post
    with open(target_file, 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("V14 Strict Layout Applied.")
else:
    print("Bounds error")

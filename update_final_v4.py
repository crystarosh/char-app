...
# --- 1. NEW generate_card_zip FUNCTION ---
new_func_code = r'''def generate_card_zip(char, manager):
    ...
    # Updated Wrap Text for Vertical Center logic if needed, but basic wrap here
    def draw_text_wrapped(draw, text, x, y, max_w, font, fill, align="left", max_h=None):
        ...
        # Vertical Center Calculation if max_h provided
        start_y = y
        ...
        if max_h:
            # Shift Y to center
            # available space = max_h
            # content space = total_h
    ...
    def draw_decorations(draw, w, h):
        # Strict Victorian Corners - REVISED LOGIC based on user feedback
        # User says "Reversed". Previously I assumed TL=Original.
        # If user says reversed, maybe Original is meant for Top RIGHT?
        # Let's try: TL = Horizontal Flip of Original. TR = Original.
        # Or maybe the image is just pointing Outwards?
        # Assuming "Victorian Corner" usually points into the center.
        # If current logic (TL=Orig) is "Reversed", maybe Orig image points OUT?
        # Let's try rotating/flipping differently.
        # Plan: TL = Flip Both (to point in?). No, let's just swap Horizontals.
        # PREV: TL=Orig, TR=FlipH.
        # NEW: TL=FlipH, TR=Orig. (Swap L/R).
        # Same for Bottom: BL=FlipBoth? BR=FlipV?
    ...
    def draw_radar(base_img, center, radius, stats, labels, color_fill, color_line):
        ...
        for j in range(n):
            ...
            r = radius * (v/5)
            # Limit break
            if v > 5: r = radius * (6/5) # Extend slightly
    ...
    def create_card_1():
        # Setup Font Color based on Race
        ...
        # Layers: 1. BG(White) -> 2. Images -> 3. Template
        ...
        # Text Layout
        # "Move Name block DOWN a bit from previous, ~30px line spacing"
        # Previous Y=480. New Y=500?
        lx = 80
        curr_y = 520 
        ...
        # Memo: Vertical Center in Bottom Box
        # Box approx y=800 to 1024? Or defined frame space?
        # Assuming bottom area starts around y=850, height ~150px.
        # Let's say max_h=120. y=860.
        memo = details.get('profile_memo', details.get('memo', ''))
        draw_text_wrapped(draw, memo, 80, 860, 860, get_font(22), txt_col, max_h=120)
        return img
    ...
    def create_card_2():
        ...
        # User Name: Higher. Previous 20. Now "Further Up"? 
        # Or was 40 -> 20? 
        # User request today: "Current position is overlapping top line. Move 20px UP."
        # If current is 20, make it 0? Or negative?
        # Let's try y=10.
        un = char.get('user_name', '')
        if un: draw.text((512, 10), f"User: {un}", font=f_small, fill="gray", anchor="mm")
        ...
        f_bio = get_font(16)
        draw_text_wrapped(draw, bio, dx, dy+40, 900, f_bio, "#212121")
        return bg
    ...
    def create_card_3():
        ...
        # Image Loading Logic - FORCE LOAD
        imgs = char.get('images', [])
        # Provide fallback if list is short
        def safe_img(idx):
...
try:
    ...
    # Detect encoding
    encoding = 'utf-8'
    if raw.startswith(codecs.BOM_UTF16_LE): encoding = 'utf-16-le'
    elif raw.startswith(codecs.BOM_UTF16_BE): encoding = 'utf-16-be'
    elif raw.startswith(codecs.BOM_UTF8): encoding = 'utf-8-sig'
    
    content = raw.decode(encoding)

    # A. REPLACE generate_card_zip
    ...
    if s_idx != -1 and e_idx != -1:
        content = content[:s_idx] + new_func_code + "\n\n" + content[e_idx:]
        print("Function generated_card_zip updated.")
    
    # B. FIX UI MAX CHARS (100 -> 150)
    # Target: `max_chars=100` in profile_memo line
    # Note: `profile_memo = st.text_area("Profile Memo (100文字以内)", ... max_chars=100 ...`
    # Replace limits and label.
    
    content = content.replace('st.text_area("Profile Memo (100文字以内)"', 'st.text_area("Profile Memo (150文字以内)"')
    content = content.replace('max_chars=100', 'max_chars=150')
    
    print("UI max_chars updated.")

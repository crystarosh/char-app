...
# THE V25 CODE
v25_code = r'''def generate_card_zip(char, manager):
    import io
    import zipfile
    import os
    ...
    # --- V25 POLISH (HQ Radar, Equal Margins, Left Name) ---
    print(f"DEBUG: V25 - {char.get('name')}")
    ...
    # --- V25: HQ Radar (Supersampling) ---
    def draw_radar(base_img, center, radius, stats, labels, color_fill, color_line):
        # Scale factor
        S = 4
        w, h = base_img.size
        # Create temp layer at 4x size relative to a bounding box? 
        # Easier to create a full size temp layer scaled up? No, too big.
        # Just create a patch around the radar.
        # Radar box ~ radius*2 + padding.
        
        # But we need to paste it back exactly.
        # Let's create a transparent layer valid for the whole image but only draw locally?
        # Or just draw on a temp image of size (Radius*2.5 * S, Radius*2.5 * S)
        
        size = int(radius * 2.5 * S)
        c_rel = (size // 2, size // 2)
        r_sc = radius * S
        
        # Temp Image (Transparent)
        # Note: Using high resolution for Anti-Aliasing
        ...
        for i in range(1, 6):
            ...
            draw.polygon(pts, outline="#AAAAAA", width=1*S) # Scale width
        ...
        for j in range(n):
            ...
            # Labels (Draw on BASE image, not HQ layer, to keep text crisp/manageable)
            # Calculate positions for labels relative to original center
            ...
            # We defer label drawing to after pasting the HQ chart
    ...
    def create_card_1():
        ...
        # V25: Memo Box calc
        # Label at 80.
        # Img 2 Right Edge at 964.
        # Memo Text Width = 964 - 80 = 884.
        
        img1_box = (125, 107, 360, 360)
        
        # Img 2 Right = 964. Width = 320. X = 644.
        img2_box = (644, 107, 320, 520)
        ...
        # V25: User name Left Aligned @, near top right
        # Trying x=800? "After the @".
        # If I draw "@" at 800, name at 800+?
        # User said: "Right top user name left alignment (as to follow the @)". 
        # I'll put "@" and Name together anchor="la" at specific X.
        un = char.get('user_name', '')
        if un: 
             draw.text((850, 40), f"@{un}", font=get_font(24), fill=txt_col, anchor="lm")
             
        memo_y = 780
        draw.text((80, memo_y), "【簡易設定】", font=get_font(20), fill=memo_col, anchor="la")
        
        # Max Height calc
        max_h = 1024 - (memo_y + 40) - 20 
        # Width 884 (to reach 964)
        draw_text_wrapped(draw, details.get('memo', ''), 80, memo_y+40, max_h, 884, get_font(18), memo_col, valign="center")
        
        return img

    # --- 2. STATS CARD (V25) ---
    def create_card_2():
        print("DEBUG: Creating Card 2 (V25)")
    ...
    # --- 3. GALLERY CARD (V25) ---
    def create_card_3():
        print("DEBUG: Creating Card 3 (V25)")
        if os.path.exists(TEXTURE_PATH):
            bg = Image.open(TEXTURE_PATH).convert("RGBA").resize(CANVAS_LANDSCAPE)
        else:
            bg = Image.new("RGBA", CANVAS_LANDSCAPE, (250, 245, 230, 255))
        ...
        # X Cols: 60, 368, 676
        # W: 288
        # Y Start: Header Y=60. Text height ~ 40? 
        # Relation Title Y=395?
        
        # Layout strategy:
        # Img 2 (Tall): Left Col (60, 100, 288, 580) approx?
        # Img 3, 4: Top row (368, 100) ...
        # But if Img 2 is Tall, it spans 2 blocks?
        # V23 map:
        # Left: Tall
        # Mid: Top (3), Bot (5)
        # Right: Top (4), Bot (Relations)
        # 
        # Let's map to cols [0, 1, 2]
        # Col 0 (Left): Image 2 (Tall)
        # Col 1 (Mid):  Image 3 (Sq), Image 5 (Sq)
        # Col 2 (Right): Image 4 (Sq), Relations (Sq)
        
        # X Coords: 60, 368, 676
        # Y Start: 100 (Frame top 40 + gap 20 = 60? No, Header is there). 
        # Header "Name" at 60. Text size 30. Ends 90. + Gap 10 = 100.
        ...
        # Frame Bottom: 728. 
        # y_start + h_tall = 100 + 596 = 696.
        # Bottom Margin = 728 - 696 = 32. Close to 20? 
        # Let's target 696 bottom.
        
        R = 25
        
        # Image 2 (Left, Tall)
        if p2:
             i = load_image_masked(p2, (w, h_tall), radius=R, centering=(0.7, 0.5))
        ...
        # Image 5 (Mid Bot)
        y_bot = y_start + h_sq + gap # 100+288+20 = 408
        if p5:
             i = load_image_masked(p5, (w, h_sq), radius=R)
             bg.paste(i, (368, y_bot), i)
        
        # Relations (Right Bot) -> at (676, 408)
        rel_x = 676
        rel_y = 408
        ...
        # Relation Title: Top of Block
        # Block starts at 408.
        draw.text((rel_x + w//2, rel_y - 20), "【メイン人間関係】", font=get_font(20), fill="#3e2723", anchor="mb") 
        
        # Relations
        # 4 items in 2x2 grid inside (w, h_sq) = (288, 288)
        # Grid: 
        # Col w: 144. Row h: 144.
        ...
        for i, r in enumerate(rels):
             ...
             draw.text((bx+72, by+20), rtype, font=get_font(14), fill="#5d4037", anchor="match")
             draw.text((bx+72, by+25), rtype, font=get_font(14), fill="#5d4037", anchor="mm")
...
if match:
    new_content = content[:match.start()] + v25_code + "\n\n" + content[match.end():]
    with open(target_file, 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("V25 Applied.")
else:
    print("Error finding block for V25.")

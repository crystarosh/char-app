...
# THE V29 CODE
v29_code = r'''def generate_card_zip(char, manager):
    import io
    import zipfile
    import os
    ...
    # --- V29 STRICT PHYSICS LAYOUT ---
    print(f"DEBUG: V29 Strict - {char.get('name')}")
    ...
    def inputs_paste_shadowed_strict(base_img, fg_img, xy, blur=10, offset=(5,5)):
        # V29: Strict Directional Shadow
        # 1. Create black silhouette
        if not fg_img: return
        
        alpha = fg_img.split()[-1]
        w, h = fg_img.size
        
        # Shadow canvas (larger for blur)
        margin = blur * 2
        sw, sh = w + margin*2, h + margin*2
        shadow_layer = Image.new("RGBA", (sw, sh), (0,0,0,0))
        
        # Draw black silhouette offset by margin
        # We want the shadow to be at (5,5) relative to image
        # So we draw it at (margin, margin) then paste layer at (x+5-margin, y+5-margin)
        
        # Create pure black image
        silhouette = Image.new("RGBA", (w, h), (0,0,0,150)) # 150 alpha black
        silhouette.putalpha(alpha) # Apply mask
        
        # Paste into shadow layer
        shadow_layer.paste(silhouette, (margin, margin), mask=alpha)
        
        # Blur
        shadow_layer = shadow_layer.filter(ImageFilter.GaussianBlur(blur))
        
        # Paste Shadow on Base: Offset (5,5)
        paste_x = xy[0] + offset[0] - margin
        paste_y = xy[1] + offset[1] - margin
        
        base_img.alpha_composite(shadow_layer, (paste_x, paste_y))
    ...
    def create_card_1():
        ...
        # V29 Logic: Equal Margins.
        # User in V28 liked "Right 964". 
        # So we set Right Edge = 964.
        # Left Margin = 1024 - 964 = 60.
        # So Right Margin should also be 60.
        # Memo Area: X=60, Width=904. (60+904=964).
        
        area_x = 60
        area_w = 904
        area_right = area_x + area_w # 964
        
        img1_box = (125, 107, 360, 360)
        
        # Image 2 Right ALign to area_right
        im2_w = 320
        im2_x = area_right - im2_w # 964 - 320 = 644
        ...
        # V29: Memo Pad 50px
        pad = 50
        text_x = area_x + pad # 60+50 = 110
        text_w = area_w - (pad*2) # 904 - 100 = 804
    ...
    # --- 2. STATS CARD (V29) ---
    def create_card_2():
        print("DEBUG: Creating Card 2 (V29)")
    ...
    # --- 3. GALLERY CARD (V29) ---
    def create_card_3():
        print("DEBUG: Creating Card 3 (V29)")
        if os.path.exists(TEXTURE_PATH):
            bg = Image.open(TEXTURE_PATH).convert("RGBA").resize(CANVAS_LANDSCAPE)
        else:
            bg = Image.new("RGBA", CANVAS_LANDSCAPE, (250, 245, 230, 255))
        ...
        # V29: Start with shadows using STRICT directional logic
        
        if p2:
             i = load_image_masked(p2, (w, h_tall), radius=R, centering=(0.5, 0.5))
             inputs_paste_shadowed_strict(bg, i, (60, y_start), blur=10, offset=(5,5))
             
        if p3:
             i = load_image_masked(p3, (w, h_sq), radius=R)
             inputs_paste_shadowed_strict(bg, i, (368, y_start), blur=10, offset=(5,5))
             
        if p4:
             i = load_image_masked(p4, (w, h_sq), radius=R)
             inputs_paste_shadowed_strict(bg, i, (676, y_start), blur=10, offset=(5,5))
             
        y_bot = y_start + h_sq + gap 
        if p5:
             i = load_image_masked(p5, (w, h_sq), radius=R)
             inputs_paste_shadowed_strict(bg, i, (368, y_bot), blur=10, offset=(5,5))
        ...
        # V29: Smaller Header (70% of Stats(30) ~ 21) => 21px
        fname = char.get('first_name', '')
        lname = char.get('last_name', '')
        full = f"{fname}・{lname}" if fname and lname else char.get('name', '')
...
if match:
    new_content = content[:match.start()] + v29_code + "\n\n" + content[match.end():]
    with open(target_file, 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("V29 Strict Applied.")
else:

...
# THE V21 CODE (Self-contained)
v21_code = r'''def generate_card_zip(char, manager):
    import io
    import zipfile
    import os
    ...
    # --- DEBUG ---
    print(f"DEBUG: V21 (Restored) - {char.get('name')}")
    ...
    # --- 2. STATS CARD (V21 Adjustment: Gap offset 45px) ---
    def create_card_2():
        print("DEBUG: Creating Card 2 (V21)")
        ...
        # Changed 35 -> 45
        draw_text_wrapped(draw, bio, dx, dy+45, 200, 764, get_font(16), "#212121")
...
# 2. Replace using Exact Boundaries
# Start: def generate_card_zip(char, manager):
# End: def main(): (or whatever is next)
# I will use a regex to be safe.

import re
# Regex to find generate_card_zip block until the next top-level def or if __name__ or EOF
# Pattern: ^def generate_card_zip.*?(?=^def |^if __name__)
pattern = re.compile(r"^def generate_card_zip.*?^(?=def |if __name__)", re.MULTILINE | re.DOTALL)

match = pattern.search(content)
if match:
    # Found it!
    new_content = content[:match.start()] + v21_code + "\n\n" + content[match.end():]
    with open(target_file, 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("V21 Restoration: Replaced function block.")
else:
    # Fallback: maybe 'def main' is not next. Just look for 'def generate_card_zip' and assume it goes until line 1250?
    # Or just use string replace if the header matches exactly
    start_str = "def generate_card_zip(char, manager):"
    pos = content.find(start_str)
    if pos != -1:
         # Find end. We saw 'def main():' at line 1266 in view_file.
         end_pos = content.find("def main():")
         if end_pos != -1:
             new_content = content[:pos] + v21_code + "\n\n" + content[end_pos:]
             with open(target_file, 'w', encoding='utf-8') as f:
                f.write(new_content)
             print("V21 Restoration: Replaced by finding main()")
         else:

...
# THE V32 CODE
v32_code = r'''def generate_card_zip(char, manager):
    import io
    import zipfile
    import os
    ...
    # --- V32 RADAR RESTORE (Alpha 80, Standard Order) ---
    print(f"DEBUG: V32 - {char.get('name')}")
    ...
    def draw_radar(base_img, center, radius, stats, labels, color_fill, color_line):
        ...
        # 1. Draw Grid Lines on poly_layer (V32: Standard Order - Grid BEHIND)
        for i in range(1, 6):
            r = r_sc * (i/5)
            pts = []
            for j in range(n):
            ...
            draw_p.polygon(pts, outline="#AAAAAA", width=1*S)

        for j in range(n):
            ang = start_angle + j * angle_step
            end = (c_rel[0] + r_sc*math.cos(ang), c_rel[1] + r_sc*math.sin(ang))
            draw_p.line([c_rel, end], fill="#AAAAAA", width=1*S)
        
        # 2. Draw Filled Polygon (V32: Standard Order)
    ...
    # --- 1. PROFILE CARD (V32 - Keep V31 Layout) ---
    def create_card_1():
    ...
    # --- 2. STATS CARD (V32 - Restore Alpha 80) ---
    def create_card_2():
        print("DEBUG: Creating Card 2 (V32)")
    ...
    # --- 3. GALLERY CARD (V32 - Keep V31 Layout) ---
    def create_card_3():
        print("DEBUG: Creating Card 3 (V32)")
        if os.path.exists(TEXTURE_PATH):
            bg = Image.open(TEXTURE_PATH).convert("RGBA").resize(CANVAS_LANDSCAPE)
        else:
            bg = Image.new("RGBA", CANVAS_LANDSCAPE, (250, 245, 230, 255))
...
if match:
    new_content = content[:match.start()] + v32_code + "\n\n" + content[match.end():]

...
# --- NEW FUNCTION CODE ---
# Using forward slashes to avoid \U escape issues
new_func_code = r'''def generate_card_zip(char, manager):
    ...
    def draw_decorations(draw, w, h):
        # V5 Logic: TL=FlipH, TR=Orig, BL=FlipBoth, BR=FlipV
    ...
    def create_card_1():
        ...
        # Color Logic: Dark Gray/Black unless Mazoku(White)
        if race == "魔族":
            txt_col = "#FFFFFF"
        else:
            txt_col = "#333333"
        ...
        # Images
        # 1. Face (Left): 450x450 at (50, 100)
        if len(char['images']) > 0:
            i1 = load_image_cover(char['images'][0], (450, 450))
            img.paste(i1, (50, 100))
            
        # 2. Body (Right): 400x650 at (560, 100) (Centered in right frame)
        ...
        # Memo: Vertical Center in 140px high box
        memo = details.get('profile_memo', details.get('memo', ''))
        draw_text_wrapped(draw, memo, 80, 860, 860, get_font(22), txt_col, max_h=140)
        return img
...
try:
    ...
    if pattern.search(content):
        ...
        with open(target_file, 'w', encoding='utf-8') as f:
            f.write(new_content)
        print("Function generate_card_zip updated successfully V5.")
    else:
        ...
        if s_idx != -1:
            ...
            e_idx = content.find(e_mark, s_idx)
            if e_idx != -1:
                new_content = content[:s_idx] + new_func_code + "\n\n" + content[e_idx:]
                with open(target_file, 'w', encoding='utf-8') as f:
                    f.write(new_content)
                print("Function updated via string fallback.")
            else:

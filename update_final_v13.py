...
# THE V13 FUNCTION
v13_code = r'''def generate_card_zip(char, manager):
    import io
    import zipfile
    import os
    ...
    def load_image_cover(path, size, radius=0):
        try:
    ...
    def create_card_1():
        ...
        # 1. IMAGES with +2px OFFSET and FIT
        # Image 1 (Square): Old(110,110) -> New(112,112). Size 400x400
        if len(char['images']) > 0:
            if char['images'][0] and os.path.exists(char['images'][0]):
                i1 = load_image_cover(char['images'][0], (400, 400))
                img.paste(i1, (112, 112))
                
        # Image 2 (Body): Old(700-720,110) -> Let's use 722, 112. Size (240, 600)
        # Note: User request V12 said 720, now V13 says 722.
        if len(char['images']) > 1:
             if char['images'][1] and os.path.exists(char['images'][1]):
                i2 = load_image_cover(char['images'][1], (240, 600))
                img.paste(i2, (722, 112))
        ...
        if os.path.exists(t_path):
            ...
            # No need to force transparent for profile usually, but harmless if white is transparent
            # If user asks later, removed. Kept standard for now.
    ...
    def create_card_3():
        ...
        # NOTE: Radius 15 for rounded corners
        # NOTE: Shifting coordinates +1 or +2 and Reducing Size by 2px to fit INSIDE frame
        # V12 Coords:
        # p2: (40, 100) 300x600 -> V13: (42, 102) 296x596
        # p3: (360, 100) 300x290 -> V13: (362, 102) 296x286
        # p4: (680, 100) 300x290 -> V13: (682, 102) 296x286
        # p5: (360, 410) 300x290 -> V13: (362, 412) 296x286
        
        radius = 15
        if p2: bg.paste(load_image_cover(p2, (296, 596), radius), (42, 102), mask=load_image_cover(p2, (296, 596), radius).split()[-1])
        if p3: bg.paste(load_image_cover(p3, (296, 286), radius), (362, 102), mask=load_image_cover(p3, (296, 286), radius).split()[-1])
        if p4: bg.paste(load_image_cover(p4, (296, 286), radius), (682, 102), mask=load_image_cover(p4, (296, 286), radius).split()[-1])
        if p5: bg.paste(load_image_cover(p5, (296, 286), radius), (362, 412), mask=load_image_cover(p5, (296, 286), radius).split()[-1])
        ...
        for i, r in enumerate(rels):
            ...
            if tgt and tgt.get('images') and tgt['images'][0] and os.path.exists(tgt['images'][0]):
                 # Reuse cached func
                 ic = load_image_cover(tgt['images'][0], (ic_sz, ic_sz), radius=10)
            else:
                 ic = Image.new("RGBA", (ic_sz, ic_sz), (220, 220, 220, 255))
            
            # Icon also masked
            if ic:
                 bg.paste(ic, (bx+40, by+20), mask=ic.split()[-1])
...
if start_pos != -1 and end_pos != -1:
    ...
    new_content = pre + v13_code + "\n\n" + post
    with open(target_file, 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("V13 Strict Update Applied.")
else:

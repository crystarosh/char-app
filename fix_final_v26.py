...
# THE V26 CODE (V25 + Fix Typo)
v26_code = r'''def generate_card_zip(char, manager):
    import io
    import zipfile
    import os
    ...
    # --- V26 FIX (Anchor Typo) ---
    print(f"DEBUG: V26 (Fix Anchor) - {char.get('name')}")
    ...
    # --- 2. STATS CARD (V26) ---
    def create_card_2():
        print("DEBUG: Creating Card 2 (V26)")
    ...
    # --- 3. GALLERY CARD (V26) ---
    def create_card_3():
        print("DEBUG: Creating Card 3 (V26)")
        if os.path.exists(TEXTURE_PATH):
            bg = Image.open(TEXTURE_PATH).convert("RGBA").resize(CANVAS_LANDSCAPE)
        else:
            bg = Image.new("RGBA", CANVAS_LANDSCAPE, (250, 245, 230, 255))
...
if match:
    new_content = content[:match.start()] + v26_code + "\n\n" + content[match.end():]
    with open(target_file, 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("V26 Fix Applied.")
else:

...
# THE V15 FUNCTION
v15_code = r'''def generate_card_zip(char, manager):
    import io
    import zipfile
    import os
    ...
    def create_card_1():
        ...
        # COLOR FIXED FOR ALL (Requested: Demon -> Dark Gray, Others -> Dark Gray)
        txt_col = "#333333"
        ...
        draw_text_wrapped(draw, memo, 80, 790, 140, 860, get_font(22), txt_col)
        return img
    ...
    def create_card_3():
        ...
        # 1. BASE: PARCHMENT TEXTURE to match Card 2
        if os.path.exists(TEXTURE_PATH):
            bg = Image.open(TEXTURE_PATH).convert("RGBA").resize(CANVAS_LANDSCAPE)
        else:
            bg = Image.new("RGBA", CANVAS_LANDSCAPE, (250, 245, 230, 255))
        ...
        # 3. TEMPLATE OVERLAY (SNS-3) as frame?
        # User said "Refer to SNS-3.png for placement" but "Same design as status screen".
        # If we just overlay SNS-3 (Transparent), it gives the white frames.
        # This seems consistent with "Refer to SNS-3".
        ...
        # 4. DECORATIONS (Corners) - Drawn ON TOP to prevent occlusion by template
        draw = ImageDraw.Draw(bg)
        draw_decorations(draw, 1024, 768)
...
if start_pos != -1 and end_pos != -1:
    ...
    new_content = pre + v15_code + "\n\n" + post
    with open(target_file, 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("V15 Update Applied.")
else:

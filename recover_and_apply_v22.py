...
# THE RECOVERED FUNCTION (Based on PYC Constants)
recovered_code = r'''def generate_card_zip(char, manager):
    import io
    import zipfile
    import os
    ...
    # --- RECOVERED VERSION (from 12:29 PYC) ---
    print(f"DEBUG: RECOVERED VERSION - {char.get('name')}")
    ...
    # --- 1. PROFILE CARD (Assuming V17/V21 Logic) ---
    def create_card_1():
    ...
    # --- 2. STATS CARD (RECOVERED From PYC Constants) ---
    def create_card_2():
        print("DEBUG: Creating Card 2 (Recovered 12:29)")
        ...
        # RECOVERED KOORDINATES
        # Chart Y: 380 (was 330)
        # Radius: 130
        # Title Y ("基礎能力"): 380 + 160? No, usually top, but pyc had (256, 540) which matches LABELs?
        # Note: (256, 540) is below chart if chart is at 380 (380+130=510). So keys are below?
        # The PYC disassembly showed "基礎能力" constant near (256, 540). 
        # But `draw_radar` function handles title usually? Or separate draw text?
        # Let's assume title is handled by separate text or `draw_radar`. 
        # In my inspection code, "基礎能力" appeared next to (256, 540).
        # Let's put Title "基礎能力" at 540? Or is that the "Attribute" labels?
        # In V21, Title "基礎能力" was at (256, 100).
        # Ah, (256, 380) is Chart CENTER.
        # Maybe title is handled externally.
        # In V21: `draw.text((256, 100), "基礎能力", ...)`
        # If I didn't see (256, 100) in constants, maybe it was changed?
        # But I saw (256, 540). Maybe TITLE moved BOTTOM?
        
        chart_y = 380 # PYC
        chart_r = 130 # PYC
        
        lbl_b = ["知力", "体力", "魔力", "防御力", "行動力", "攻撃力", "自信"]
        draw.text((256, 100), "基礎能力", font=f_med, fill="#3e2723", anchor="mm") # Assuming kept unless found otherwise
        ...
        # Bio: PYC Showed 60, 600, 904.
        dx = 60
        dy = 600 # Moved DOWN
        draw.text((dx, 560), "【詳細概要】", font=f_med, fill="#3e2723") # Assuming title is above Bio (gap 40)
        bio = char.get('bio_short', '') or char.get('bio', '')[:250]
        
        # Use PYC Width: 904
        draw_text_wrapped(draw, bio, dx, dy, 200, 904, get_font(16), "#212121")
        
        draw.text((512, 750), "Legend of Crystarosh", font=get_font(12), fill="#5d4037", anchor="mm")
        
        return bg

    # --- 3. GALLERY CARD (RECOVERED) ---
    def create_card_3():
        print("DEBUG: Creating Card 3 (Recovered)")
        if os.path.exists(TEXTURE_PATH):
            bg = Image.open(TEXTURE_PATH).convert("RGBA").resize(CANVAS_LANDSCAPE)
        else:
            bg = Image.new("RGBA", CANVAS_LANDSCAPE, (250, 245, 230, 255))
        ...
        # TITLE CHANGE RECOVERED: 【メイン人間関係】
        ...
        for i, r in enumerate(rels):
            if i>=4: break
            # Logic seems similar, skipping detailed reconstruction of minor tweaks unless critical
            bx, by = positions[i]
...
if match:
    new_content = content[:match.start()] + recovered_code + "\n\n" + content[match.end():]
    with open(target_file, 'w', encoding='utf-8') as f:
        f.write(new_content)
    print("Recovered Code Applied.")
else:
    print("Error: Could not find block to replace.")
    # Fallback
    start_str = "def generate_card_zip(char, manager):"
    pos = content.find(start_str)
    if pos != -1:
         end_pos = content.find("def main():")
         if end_pos != -1:
             new_content = content[:pos] + recovered_code + "\n\n" + content[end_pos:]
             with open(target_file, 'w', encoding='utf-8') as f:
                f.write(new_content)
             print("Recovered Code Applied (Fallback).")

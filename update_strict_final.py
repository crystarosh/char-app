...
new_func = r'''def generate_card_zip(char, manager):
    import io
    import zipfile
    import os
    ...
    def draw_text_wrapped(draw, text, x, y, max_w, font, fill):
        ...
        # Estimate char width based on font size (approximate for wrapping)
    ...
    def draw_decorations(draw, w, h):
        # Strict Victorian Corners
        # User says "Reversed". I will use Standard Orientation (TL points IN).
    ...
    def create_card_1():
        ...
        # 2. Images (Behind Template) - Masked/Clipped
        # User Req: "Template Front, Character Back, MasK/Clip overflow"
        # Since we don't have exact frame path, we rely on coordinate placement and hoping Template covers edges.
        # But if transparent template doesn't have "holes" but just transparency, then placing behind works.
        # Img 1 (Face): Left
        ...
        # 4. Data Draw
        # User Req: "Move Name, Class, H/W, Affiliation UP by 80px"
        # Previous Ys: Name(560), Class(640)?
        # Let's set new Ys.
        # Start Name at y=480? (Previous was 560, minus 80 = 480)
        ...
        # Central Data (Age, Race, Eye, Hair)
        # x=530, y=130?
        cx = 530
        cy = 130
        step = 60
        ...
        draw_text_wrapped(draw, memo, 80, 850, 860, get_font(22), txt_col)
    ...
    def create_card_2():
        ...
        # User Req: "User Name 20px Higher"
        # Previous 40 -> 20.
        un = char.get('user_name', '')
        if un: draw.text((512, 20), f"User: {un}", font=f_small, fill="gray", anchor="mm")
        ...
        # Charts
        # Req: "Basic Left, Pers Right"
        # Req: "Title 30px above Chart TOP"
        # Chart Center y=300? Radius 120. Top=180. 
        # Title y should be 180 - 30 = 150.
    ...
    def create_card_3():
        ...
        # Images Mapping (Strict)
        # Img 3 -> list[2] -> Left (x=40, y=100)
        # Img 4 -> list[3] -> Center Top (x=360, y=100)
        # Img 5 -> list[4] -> Right Top (x=680, y=100)
        # Img 6 -> list[5] -> Center Bot (x=360, y=410)
        
        imgs = char.get('images', [])
        
        # Helper to get safe index
        def get_img(idx):
            if idx < len(imgs) and imgs[idx]: return imgs[idx]
            return None
            
        # 3 (Left) - Tall
        p = get_img(2)
        if p:
            bg.paste(load_image_cover(p, (300, 600)), (40, 100))
            
        # 4 (Center Top) - Square-ish
        p = get_img(3)
        if p:
            bg.paste(load_image_cover(p, (300, 290)), (360, 100))
            
        # 5 (Right Top)
        p = get_img(4)
        if p:
            bg.paste(load_image_cover(p, (300, 290)), (680, 100))
            
        # 6 (Center Bot)
        p = get_img(5)
        if p:
            bg.paste(load_image_cover(p, (300, 290)), (360, 410))
        ...
        # Title: "【人間関係】"
        draw.text((680, 400), "【人間関係】", font=f_med, fill="#3e2723")
        
        # Area Start. y=440?
        rels = char.get('relations', [])[:4]
        # 2x2 grid. x=680/830. y=440/580?
        ...
        for i, r in enumerate(rels):
            ...
            # Center icon in 150w column -> 75 - 35 = 40
